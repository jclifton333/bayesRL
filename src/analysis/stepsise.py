...
def rollout_stepwise_linear_mab(zeta, env, J=10, mc_rep=500, time_horizon=100):
  mean_cum_reward = 0.0
  for rep in range(mc_rep):
    env.reset()
    for t in range(T):
      epsilon = tune_stepwise_linear_mab(zeta, J, t, time_horizon=100)
...
def tune_stepwise_linear_mab(zeta, J, t, time_horizon=100):
  ## zeta: parameter vector with length J
  interval = int(time_horizon/J)
  j = int(t/interval)
  epsilon = sum(zeta[:j]) + (t - j*interval) * zeta[j]
  return epsilon
