...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 30 16:20:55 2019
...
class Glucose(object):
  def __init__(self, nPatients=1, sigma_eps = 0.5, mu0 = 7.7, 
...
#    eps = np.random.normal(scale=0.1, size=1)
  ...
  def get_next_state(self,prev_state,action, i):
    ...
    if prev_state[0]<4:
      ...
      A1c_t=self.get_A1c(prev_state,action,d_t,np.random.normal(0,self.sigma_eps,1), i)[0]
  ...
  # If NAT>4, make algorithm indiffernt between action 1 and 0 to enable better state space exploration
      ...
      A1c_t=self.get_A1c(prev_state,0,d_t,np.random.normal(0,self.sigma_eps,1), i)[0]
...
#         p=Glucose.prob_L_given_trts[0]
  ...
  def get_A1c(self,prev_state,action,d_t,eps, i):
      ## i: the ith patient
    if prev_state[2]>7 and prev_state[0]<4 and action!=0 and d_t!=1:
      new_u_t=0
      if prev_state[0]==0:
        new_u_t=self.prev_u_t[i]*(1-self.tau_trts[0])
      elif prev_state[0]==1:
        new_u_t=self.prev_u_t[i]*(1-self.tau_trts[1])
      elif prev_state[0]==2:
        new_u_t=self.prev_u_t[i]*(1-self.tau_trts[2])
      elif prev_state[0]==3:
        new_u_t=self.prev_u_t[i]*(1-self.tau_trts[3])
      A1c_t=(prev_state[2]-self.prev_u_t[i]+eps)/math.sqrt(1+self.sigma_eps**2)+new_u_t
#      print(str(A1c_t)+' '+str(prev_state[2])+ " Kt=1 "+str(new_u_t)+' '+str(self.prev_u_t))
      self.prev_u_t[i]=new_u_t
      return A1c_t
    else:
      A1c_t=(prev_state[2]-self.prev_u_t[i]+eps)/math.sqrt(1+self.sigma_eps**2)+self.prev_u_t[i]
      return A1c_t
  ...
  def reset(self):
    ...
    self.prev_u_t = [self.mu0] * self.nPatients
