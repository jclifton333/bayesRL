...
def rewards_policy(rep, gamma, gamma_type, K, env, T, policy="eps-greedy", epsilon=0.05, fixed_decay=0, regr="RF",
            rollouts=100, gammas=np.arange(0,1,0.1)):
    np.random.seed(rep)
    ...
    for t in range(T):
        if done:
            ...
            #        gamma = np.exp((t+1-T)/10)
                    ...
                    gamma = policy_tune_gamma(env, gammas, K, T, rollouts=rollouts, regr=regr)
                    print(t, rewards, gamma)
...
def policy_tune_gamma(env, gammas, K, T, rollouts, regr):
  ...
  for i in range(rollouts):
    ...
    for j in range(len(gammas)):
      ...
      gamma = gammas[j]
      rewards_gammas[i,j] = rewards_policy(rep, gamma, gamma_type, K, env_rollout, 
                    T, regr=regr)["mean_rewards"] 
  rewards_gammas_means = np.mean(rewards_gammas, axis=0)
  print(rewards_gammas_means)
  return(gammas[np.argmax(rewards_gammas_means)])
