...
#    with open("Ashkan_tune"+str(regr)+"_K_"+str(K)+"_T_"+str(T)+".txt", "wb") as fp:
#        pickle.dump(tune, fp)
...
#    print(np.std(np.vstack([i["gammas_used"] for i in tune]), axis=0))
    ...
    pl.close()
    with open("Ashkan_prespecify_"+str(regr)+"_K_"+str(K)+"_T_"+str(T)+".txt", "wb") as fp:
