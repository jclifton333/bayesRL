...
from src.environments.FiniteMDP import FiniteMDP
...
def finite_mdp_epsilon_policy(optimal_action, tuning_function, tuning_function_parameter, 
                              time_horizon, t, env):
  epsilon = tuning_function(time_horizon, t, tuning_function_parameter)
  if np.random.rand() < epsilon:
    optimal_action = np.random.choice(env.nA)
  return optimal_action

def initial_reward_mat(env, init_reward=50):
  return  np.ones((env.nA, env.nS, env.nS))*init_reward

def finite_mdp_rollout(tuning_function_parameter, mdp_grid_epsilon_policy, 
                             time_horizon, tuning_function, env, 
                             gamma, mc_replicates, rewardMatrices):
  mean_cumulative_reward = 0
  nS, nA = env.nS, env.nA
  transitionMatrices = np.zeros((nA, nS, nS))
  for rep in range(mc_replicates):
    ## Sample from posterior distribution (which is still a Dirichlet distribution) 
    ## to get the transition probability and then get the sim environment
    for i in range(nS):
      for j in range(nA):
        transitionMatrices[j, i, :] = np.random.dirichlet(env.posterior_alpha[j, i, :])
    sim_env = FiniteMDP(transitionMatrices=transitionMatrices, time_horizon=time_horizon, rewardMatrices=rewardMatrices)
    r = 0
    s = sim_env.reset()
    update_reward_mat = initial_reward_mat(env)
    while sim_env.t < sim_env.time_horizon:  
      update_transitionMatrices = sim_env.posterior_mean_model()
      mdp = FiniteMDP(transitionMatrices=update_transitionMatrices, time_horizon=time_horizon, rewardMatrices=update_reward_mat)
      _, pis, Q = policy_iteration(mdp)
      optimal_action = pis[-1][s]
      s_prev = sim_env.current_state
#        optimal_action = np.argmax(Q[s, ])
      action = finite_mdp_epsilon_policy(optimal_action, tuning_function, tuning_function_parameter, 
                                       sim_env.time_horizon, sim_env.t, sim_env)
      s, reward = sim_env.step(action)
      update_reward_mat[action, s_prev, s] = reward 
      r += reward
...
def bayesopt(rollout_function, policy, tuning_function, zeta_prev, time_horizon, env, mc_replicates,
             bounds, explore_, gamma, rewardMatrices):
  ...
  def objective(zeta0, zeta1, zeta2):
    ...
    return rollout_function(zeta, policy, time_horizon, tuning_function, 
                            env, gamma, mc_replicates, rewardMatrices)
...
def rollout_under_true_model(tuning_function_parameter, policy, 
                             time_horizon, tuning_function, gamma, mc_replicates):
  mean_cumulative_reward = 0
#  done=False
  for rep in range(mc_replicates):
    env = FiniteMDP(time_horizon=time_horizon)
    s = env.reset()
    r = 0
    update_reward_mat = initial_reward_mat(env)
    while env.t < env.time_horizon:  
#      print(env.counter, r)
      update_transitionMatrices = env.posterior_mean_model()
      mdp = FiniteMDP(transitionMatrices=update_transitionMatrices, 
                      time_horizon=time_horizon, rewardMatrices=update_reward_mat)
      _, pis, Q = policy_iteration(mdp)
#        optimal_action = np.argmax(Q[s, ])
      optimal_action = pis[-1][s]
      s_prev = mdp.current_state
      action = policy(optimal_action, tuning_function, tuning_function_parameter, 
                                       env.time_horizon, env.t, env)
...
#    explore_ = {'zeta0': [1.0, 0.05, 1.0, 0.1], 'zeta1': [50.0, 49.0, 1.0, 49.0], 'zeta2': [0.1, 2.5, 1.0, 2.5]}
    rollout_function = finite_mdp_rollout
...
#    tuning_function_parameter = np.array([5.47368649e+00, 1.23051916e+02, 6.69618550e-02])
  ...
  update_reward_mat = initial_reward_mat(env)
  while env.t < env.time_horizon:
    print("########### timestep {}, rewards {} #########".format(env.t, sum(rewards)))
    s = env.reset()
    acts=[]
