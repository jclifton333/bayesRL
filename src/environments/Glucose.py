...
#    self.horizon = horizon
  ...
  def reset(self):
    ...
    self.SX = [[]]*self.nPatients # List of states corresponding to the states in MDP, exclude current action in self.X, and 1-lag advance than self.X
...
#    print(self.current_state[i], self.last_action[i])
    ...
    if len(self.COEF) == 5:
      glucose = np.random.normal(np.dot(np.delete(x, [4,5,6,7]), self.COEF), self.SIGMA_NOISE)
    else:
